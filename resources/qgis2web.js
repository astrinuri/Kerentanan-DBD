...
map.getView().fit([12238710.464041, -929399.922796, 12388431.107554, -822928.634640], map.getSize());
    ...
    for (var i = 0; i < currentFeatureKeys.length; i++) {
        if (currentFeatureKeys[i] != 'geometry') {
            ...
            if (layer.get('fieldImages')[currentFeatureKeys[i]] != "ExternalResource") {
                let fieldName = currentFeatureKeys[i];
                let value = currentFeature.get(fieldName);

                // Mapping khusus untuk field tertentu
                if (fieldName === "prediksi_y") {
                    if (value === 1) value = "Rendah";
                    else if (value === 2) value = "Sedang";
                    else if (value === 3) value = "Tinggi";
                }

                // Pembulatan SGDVI
                const sgdviFields = ["entropy", "entropy_me", "entropy_mean"];

                if (sgdviFields.includes(fieldName)) {
                    value = parseFloat(value).toFixed(3);
                }

                popupField += (value != null ? autolinker.link(value.toString()) + '</td>' : '');
            } else {
